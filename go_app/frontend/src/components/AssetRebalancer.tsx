...
export const AssetRebalancer: React.FC<Props> = ({ data: propData, defaultMonthlyAddition: propDefaultMonthlyAddition, pastMilestones: propPastMilestones = [], pastUSDMilestones: propPastUSDMilestones = [] }) => {
        ...
        api.getRebalancerConfig().then(configStr => {
            if (configStr) {
                try {
                    ...
                    if (config.stepUpMonth) setStepUpMonth(config.stepUpMonth);
    ...
    if (pendingETA.size > 0) {
        ...
        const targetMonth = parseInt(stepUpMonth) || 1;

        for (let m = 1; m <= ETA_MAX_MONTHS && pendingETA.size > 0; m++) {
            const simDate = new Date();
            simDate.setMonth(simDate.getMonth() + m);

            // Apply annual step-up to SIP directly on the target calendar month
            if (m > 0 && (simDate.getMonth() + 1) === targetMonth) {
                rebalanceData.forEach(item => {
    ...
    const growthProjectionData = useMemo(() => {
        ...
        for (let m = 1; m <= nMonths; m++) {
            ...
            if (m > 0 && (futureDate.getMonth() + 1) === targetMonth) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white p-4 rounded border border-indigo-200">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-2">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Step-Up Month</label>
                                <select
                                    value={stepUpMonth}
                                    onChange={(e) => setStepUpMonth(e.target.value)}
                                    className="block w-full border border-gray-300 rounded-md shadow-sm p-2 text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 bg-white"
                                >
